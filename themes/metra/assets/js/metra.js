...
function showRules(gamemode) {
    if (rules[gamemode] instanceof Array === false) {
        gamemode = 'global';
    }
...
function showStaff(gamemode) {
    if (Array.isArray(staff[gamemode]) === false) {
        gamemode = 'global';
    }
        ...
        if (staff[gamemode].length > 0) {
            ...
            for (var i = 0; i < staff_blocks; i++) {
                ...
                for (var x = 0; x < 2; x++) {
                    ...
                    real_cnt++;
                    if (typeof staff_member === 'undefined') {
                        continue;
                    }
                    ...
                    children.push(
                        elem('div', {className: 'member'}, [
                            elem('img', {
                                ...
                                src: site.path + '/api/player/' + steamid + '/avatarmedium?raw'
                            }),
                            elem('div', {}, [
