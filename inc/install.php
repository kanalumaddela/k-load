...
<div class="container">
    ...
    <div class="card z-depth-2">
        <a href="<?= APP_PATH ?>/install?phpinfo" target="_blank"
           style="position:absolute;right:14px;top:8px;">phpinfo</a>
                    ...
                    <li id="requirements" class="step active">
                        ...
                        <div class="step-content">
                            <div class="row">
                                <div class="desc">
                                    <p>
                                        This is a list of extensions that are REQUIRED for K-Load to run. The majority
                                        of webhosts should have these extensions
                                        installed, but if not, try contacting them and getting it installed. If you are
                                        self hosted, you should know how to install
                            ...
                            <div class="step-content">
                                <div class="row">
                                    <div class="col s12 desc">
                                        <p>
                                            During the initial setup, the 1st person to login and run the install
                                            becomes the root admin. This
                                            is saved to the <code>data/config.php</code> file to ensure you'll always
                                            have access and can't be locked out (unless, of course, your config.php)
                                            is altered.
                                        </p>
                                        <p>
                                            Here are some general settings that are required before running K-Index. You
                                            can visit steam's site to get
                                            your api key by clicking the "Get Key" button. This key is primarily used to
                                            retrieve a player's steam info such
                                            as their steam name, avatar, and profile. This is used for member, staff,
                                            and other areas in the site.
                                        </p>
                                    </div>
                                    <div class="input-field col s12 m6">
                                        <i class="mdi mdi-steam prefix"></i>
                                        <input required type="number" id="admin_id" name="admins[]"
                                               value="<?= $_SESSION['steamid'] ?>" readonly="readonly">
                                        <label for="admin_id">Admin SteamID</label>
                                    </div>
                                    <div class="input-field col s12 m8">
                                        <i class="mdi mdi-key prefix"></i>
                                        <input required type="text" id="steam_apikey" name="apikeys[steam]"
                                               value="<?= $config['apikeys']['steam'] ?? '' ?>">
                                        <label for="steam_apikey">Steam API Key</label>
                                    </div>
                                    <div class="input-field col s12 m4">
                                        <a href="https://steamcommunity.com/dev/apikey" target="_blank" class="btn">Get
                                            Key</a>
                                    </div>
                                </div>
                                <div class="step-actions">
                                    <button class="waves-effect waves-dark btn next-step" data-feedback="steamTest">
                                        Next
                                    </button>
                        ...
                        <li id="mysql" class="step">
                            ...
                            <div class="step-content">
                                <div class="row">
                                    <div class="col s12 desc">
                                        <p>
                                            Create the database beforehand and give it a separate user account with
                                            access ONLY to that database. DO NOT use the mysql root account.
                                            I have confidence in my code, but it's always better to take security
                                            precautions no matter what.
                                        </p>
                                    </div>
                                    <div class="input-field col s12 m9">
                                        <i class="mdi mdi-server-network prefix"></i>
                                        <input required type="text" id="mysql_host" name="mysql[host]" class="validate"
                                               placeholder="localhost"
                                               value="<?= $config['mysql']['host'] ?? 'localhost' ?>">
                                        <label for="mysql_host">MySQL Host</label>
                                    </div>
                                    <div class="input-field col s12 m3">
                                        <i class="mdi mdi-ethernet prefix"></i>
                                        <input required type="number" id="mysql_port" name="mysql[port]"
                                               class="validate" placeholder="3306"
                                               value="<?= $config['mysql']['port'] ?? 3306 ?>">
                                        <label for="mysql_port">MySQL Port</label>
                                    </div>
                                    <div class="input-field col s12 m6">
                                        <i class="mdi mdi-account-network prefix"></i>
                                        <input required type="text" id="mysql_user" name="mysql[user]" class="validate"
                                               placeholder="root" value="<?= $config['mysql']['user'] ?? 'root' ?>">
                                        <label for="mysql_user">MySQL User</label>
                                    </div>
                                    <div class="input-field col s12 m6">
                                        <i class="mdi mdi-textbox-password prefix"></i>
                                        <input required type="password" id="mysql_pass" name="mysql[pass]"
                                               class="validate" placeholder="*********"
                                               value="<?= $config['mysql']['pass'] ?? '' ?>">
                                        <label for="mysql_pass">MySQL Pass</label>
                                    </div>
                                    <div class="input-field col s12 m6">
                                        <i class="mdi mdi-database prefix"></i>
                                        <input required type="text" id="mysql_db" name="mysql[db]" class="validate"
                                               placeholder="k-load" value="<?= $config['mysql']['db'] ?? 'k-load' ?>">
                                        <label for="mysql_db">MySQL Database</label>
                                    </div>
                                </div>
                                <div class="step-actions">
                                    <button type="button" class="waves-effect waves-dark btn blue next-step"
                                            data-feedback="mysqlTest">Next
                                    </button>
                                    <button type="button" class="waves-effect waves-dark btn-flat previous-step">
                                        Previous
                                    </button>
                                </div>
                            </div>
